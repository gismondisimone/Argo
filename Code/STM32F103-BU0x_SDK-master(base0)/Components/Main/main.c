...
static uint8_t seq; static uint32_t distance_mm[2];
static void waitms(uint32_t n){while(n--)Sleep(1);}
...
int main(void){uint8_t f[FRAME_LEN_MAX];SystemInit();Hal_Driver_Init();uart_init();port_set_dw_ic_spi_fastrate();reset_DWIC();waitms(2);while(!dwt_checkidlerc()){}if(dwt_initialise(DWT_DW_INIT)==DWT_ERROR)while(1){}if(dwt_configure(&cfg)==DWT_ERROR)while(1){}dwt_configuretxrf(&txconfig_options);dwt_setrxantennadelay(RXAD);dwt_settxantennadelay(TXAD);while(1){uint32_t st;dwt_setrxtimeout(0);dwt_setpreambledetecttimeout(0);dwt_rxenable(DWT_START_RX_IMMEDIATE);do{st=dwt_read32bitreg(SYS_STATUS_ID);}while(!(st&(SYS_STATUS_RXFCG_BIT_MASK|SYS_STATUS_ALL_RX_ERR)));if(st&SYS_STATUS_RXFCG_BIT_MASK){uint16_t n=dwt_read32bitreg(RX_FINFO_ID)&RX_FINFO_RXFLEN_BIT_MASK;uint64_t prx=rxts();dwt_write32bitreg(SYS_STATUS_ID,SYS_STATUS_RXFCG_BIT_MASK);if(n<=FRAME_LEN_MAX){dwt_readrxdata(f,n,0);if(n>=12&&f[3]==(uint8_t)PAN&&f[4]==(PAN>>8)){if(f[9]==POLL&&f[5]==(uint8_t)ME&&f[6]==(ME>>8))twr_response(prx);else if(f[9]==RELAY&&n>=16){memcpy(&distance_mm[1],f+10,4);uart_send();}else if(f[9]==BUTTON)display(f[10]==2?2:1);}}}else dwt_write32bitreg(SYS_STATUS_ID,SYS_STATUS_ALL_RX_ERR);}}
